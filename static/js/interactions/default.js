    ...
    function set_widget_defaults(id, mechanic_id, link_url){
        ...
        .then(res_json => (console.log(res_json), document.querySelector(id).innerHTML += '<style>.grow { transition: all .2s ease-in-out; }' +
        										...
        										'<p style="position: absolute; top: 0; right: 0;font-size:calc(10px + 1vw);width:90%;"> ' + 
        											'<img style="width:10%;float:right; right:calc(30px + 5vw)" src="https://agmodule.herokuapp.com/media/avatars/' + res_json.avatar.slice(res_json.avatar.indexOf(".") + 1, res_json.avatar.length) + '.png">' +
